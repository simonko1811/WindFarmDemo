...
public class PersonResource {
    ...
    @ApiOperation(value = "Find person by ID", response = Person.class, authorizations = {@Authorization(value = "oauth2",
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({Role.ADMIN, Role.USER_READ_ONLY})
    @ApiOperation(value = "Save or update person", response = Person.class, authorizations = {@Authorization(value = "oauth2",
            scopes = {@AuthorizationScope(scope = Role.ADMIN, description = "Access to all resources"), @AuthorizationScope(scope = Role.USER_READ_ONLY, description = "Limited access")})})
